...
describe('Admin (API test)', () => {
  ...
  */
  ...
  it(`POST /admin/send_email_notifications n'envoie pas le welcome`, async () => {
    // GIVEN
    TestUtil.token = process.env.CRON_API_KEY;
  ...
  it(`POST /admin/send_email_notifications envoie pas le mail late_onboarding`, async () => {
    // GIVEN
    TestUtil.token = process.env.CRON_API_KEY;
  ...
  it('POST /admin/send_welcome_email envoie le mail de bienvenu', async () => {
    // GIVEN
    TestUtil.token = process.env.CRON_API_KEY;
    process.env.NOTIFICATIONS_MAIL_ACTIVES = 'welcome';
    await TestUtil.create(DB.utilisateur);
    ...
    // WHEN
    const response = await TestUtil.POST('/admin/send_welcome_emails');
  ...
  it('POST /admin/send_welcome_email pas de welcome 2 fois de suites ^^', async () => {
    // GIVEN
    TestUtil.token = process.env.CRON_API_KEY;
    process.env.NOTIFICATIONS_MAIL_ACTIVES = 'welcome';
    await TestUtil.create(DB.utilisateur);
    ...
    // WHEN
    const response2 = await TestUtil.POST('/admin/send_welcome_emails');
