...
describe('Actions (API test)', () => {
  ...
  it(`GET /utilisateurs/id/actions/id - accorche uniquement les aides non-expirées`, async () => {
    // GIVEN
    await TestUtil.create(DB.utilisateur, { logement: logement as any });
    await TestUtil.create(DB.action, {
      code: '123',
    ...
    await TestUtil.create(DB.aide, {
      ...
      echelle: Echelle.Commune,
      codes_commune_from_partenaire: ['21231'],
      date_expiration: new Date('2023-10-01'),
    });
    await TestUtil.create(DB.aide, {
      content_id: '2',
      besoin: 'composter',
    ...
    expect(action.aides).toHaveLength(1);
    expect(action.aides[0].content_id).toEqual('2');
    expect(action.nom_commune).toEqual('Dijon');
  });
