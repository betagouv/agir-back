...
describe('/dashboard (API test)', () => {
  ...
  it('GET /dashboards/id - get a dashboard by id with proper compteurs, badges, quizz', async () => {
    ...
    await TestUtil.prisma.dashboard.create({
      data: { id: '123', utilisateurId: '1', todoQuizz: ['quizzID'] },
    });

    await TestUtil.prisma.compteur.create({
      data: { id: '1', titre: 't1', valeur: '1', dashboardId: '123' },
    });
    await TestUtil.prisma.compteur.create({
      data: { id: '2', titre: 't2', valeur: '99', dashboardId: '123' },
    });

    await TestUtil.prisma.badge.create({
      data: { id: '1', titre: 'badge', date: new Date(), dashboardId: '123' },
    });

    const response = await TestUtil.getServer().get('/dashboards/1');
