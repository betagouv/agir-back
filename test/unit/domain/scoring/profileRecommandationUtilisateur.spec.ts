...
describe('ProfileRecommandationUtilisateur', () => {
  ...
  it('trierEtFiltrerRecommandations : tag recommandation_winter', () => {
    // GIVEN
    const content: TaggedContent = {
      score: 0,
      pourcent_match: 0,
      ...
      getDistinctText: () => 'abc',
      isLocal: () => false,
      getInclusionTags: () => [Tag_v2.recommandation_winter],
      getExclusionTags: () => [],
      getThematiques: () => [Thematique.alimentation],
      explicationScore: new ExplicationScore(),
    };
    ...
    expect(Math.round(content.pourcent_match)).toEqual(100);
  });
